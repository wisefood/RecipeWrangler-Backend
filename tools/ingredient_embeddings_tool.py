# tools/ensure_ingredients_in_collection.py
...
def _stable_id(name: str) -> str:
    """Deterministic ID from the ingredient name (avoids duplicate rows across runs)."""
...
def _find_existing_by_name(col, names: list[str]) -> dict[str, tuple[str, dict]]:
    ...
    try:
        ...
        # Fallback per-name
        for name in names:
            r = col.get(where={"name": name}, include=["metadatas", "documents"])  # <-- drop "ids"
            if r.get("ids"):
                existing[name] = (r["ids"][0], (r.get("metadatas") or [{}])[0])
        return existing


@tool("ensure_ingredients_in_collection", return_direct=False)
