...
def rule_based_search_tool(title: str) -> dict:
    ...
    Returns a list of recommended recipes with scores under 'recommendations'.
    """
    ...
    RETURN
        ...
        r2.title AS recommended_recipe,
        finalScore
    ORDER BY finalScore DESC
    LIMIT 10
    """

    recommendations = run_query(query, parameters={"title": title})
    recommended_titles = [record['recommended_recipe'] for record in recommendations]
...
def Rule_Based_Similar_Recipes_Node(state: dict) -> dict:
    ...
    if isinstance(result, dict) and "recommendations" in result:
        state["similar_recipes"] = result["recommendations"]
    else:
