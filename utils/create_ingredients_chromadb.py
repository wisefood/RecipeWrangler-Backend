...
# --- config ---
PERSIST_PATH = "chroma_db"          # single folder for all your Chroma collections
...
CSV_PATH = "data/pp_recipes_curated_tagged.csv"
...
# your project tools
from tools.ingredient_cleaner import ingredient_cleaning_tool
...
def embed_batch(texts: List[str]) -> List[List[float]]:
    """
    Simple batch wrapper around your get_qwen_embeddings (single-string).
    If you have a native batch function, replace this.
    """
    vecs: List[List[float]] = []
    for t in texts:
        v = get_qwen_embeddings(t)
        if not isinstance(v, list):
            raise ValueError("get_qwen_embeddings must return a List[float]")
        vecs.append(v)
    return vecs
...
def build_chroma_collection(
    ...
    # Prefer PersistentClient so data is written once and re-used across runs
    ...
    # Persist (PersistentClient persists automatically, but this is explicit)
    print(f"[done] Upserted {added} items into collection '{collection_name}'. Final count: {col.count()}")
    print(f"[path] Persisted under: {os.path.abspath(persist_path)}")
...
if __name__ == "__main__":
    ...
    for ing in tqdm(unique_ingredients, desc="Cleaning ingredients", unit="ingredient"):
        ...
        # tool returns a list; we passed a single ingredient, so take first if present
        if out:
            cleaned_all.append(out[0])
